...
public final class ParsingSQLRouter implements SQLRouter {
    ...
    public SQLRouteResult route(final String logicSQL, final List<Object> parameters, final SQLStatement sqlStatement) {
        ...
        if (routingResult instanceof CartesianRoutingResult) {
            //            遍历数据源
            for (CartesianDataSource cartesianDataSource : ((CartesianRoutingResult)routingResult)
                .getRoutingDataSources()) {
                //               遍历笛卡尔积表路由组
            ...
            for (TableUnit each : routingResult.getTableUnits().getTableUnits()) {
                result.getExecutionUnits()
                    .add(new SQLExecutionUnit(each.getDataSourceName(), rewriteEngine.generateSQL(each, sqlBuilder)));
            }
        }
        MetricsContext.stop(context);
        if (showSQL) {
    ...
    private RoutingResult route(final List<Object> parameters, final SQLStatement sqlStatement) {
        ...
        if (1 == tableNames.size() || shardingRule.isAllBindingTables(tableNames)) {
            routingEngine =
                new SimpleRoutingEngine(shardingRule, parameters, tableNames.iterator().next(), sqlStatement);//单表路由
        } else {
            // TODO 可配置是否执行笛卡尔积
    ...
    private void processGeneratedKey(final List<Object> parameters, final InsertStatement insertStatement,
    ...
    private void processLimit(final List<Object> parameters, final SelectStatement selectStatement,
        final boolean isSingleRouting) {
        //        select语句排序项不为空    或者聚合选择项不为空  或者排序项和分组项不一致
        boolean isNeedFetchAll =
            (!selectStatement.getGroupByItems().isEmpty() || !selectStatement.getAggregationSelectItems().isEmpty())
