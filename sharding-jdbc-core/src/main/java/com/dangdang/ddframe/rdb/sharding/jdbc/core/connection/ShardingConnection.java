...
public final class ShardingConnection extends AbstractConnectionAdapter {
    ...
    @Getter
    private final ShardingContext shardingContext;//执行上下文对象

    private final Map<String, Connection> connectionMap = new HashMap<>();//缓存数据源对象
    ...
    public Collection<Connection> getConnectionForDDL(final String dataSourceName) throws SQLException {
        final Context metricsContext =
            MetricsContext.start(Joiner.on("-").join("ShardingConnection-getConnectionForDDL", dataSourceName));
        //        从分片规则的数据库分片规则中获取数据源
    ...
    private Optional<Connection> getCachedConnection(final String dataSourceName, final SQLType sqlType) {
        String key = connectionMap.containsKey(dataSourceName) ? dataSourceName :
    ...
    public DatabaseMetaData getMetaData() throws SQLException {
        return getConnection(
            shardingContext.getShardingRule().getDataSourceRule().getDataSourceNames().iterator().next(), SQLType.DQL)
            .getMetaData();
    }
